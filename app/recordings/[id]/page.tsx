...
function SummarySection({
  ...
  return (
    <details open className="group border-b last:border-b-0">
      <summary className="flex cursor-pointer list-none items-center gap-2.5 py-4 text-sm font-medium marker:hidden">
  ...
  return (
    <div className="flex min-h-[calc(100vh-3rem)] flex-col xl:h-screen xl:min-h-0 xl:overflow-hidden">
      <header className="shrink-0 border-b">
        <div className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6 xl:px-8 xl:py-7">
          <Link
            href="/"
      ...
      <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(0,1fr)_minmax(28rem,42%)] 2xl:grid-cols-[minmax(0,1fr)_minmax(32rem,42rem)]">
        <main className="min-w-0 px-4 py-6 sm:px-6 sm:py-8 xl:overflow-y-auto xl:px-10 xl:py-10 2xl:px-12">
              ...
              <div className="text-sm">
                ...
                {enhancement.keyPoints.length > 0 && (
                  <SummarySection title="Key points" icon={RiListUnordered}>
                    <ul className="space-y-2.5 pl-4 text-sm text-muted-foreground marker:text-border">
                      {enhancement.keyPoints.map((point, index) => (
                        <li key={index} className="list-disc pl-1 leading-6">
                          {point}
                        </li>
                      ))}
                    </ul>
                      ...
                      {enhancement.actionItems.map((item, index) => (
                        ...
                        >
                          ...
                          <span className="min-w-0 text-[13px] leading-5">
                            ...
                            {item.due && (
                              <span className="mt-1.5 ml-2 inline-block text-[11px] text-muted-foreground">
                                {item.due}
                              </span>
                            )}
                          </span>
        ...
        <aside
          ...
          className="min-w-0 border-t bg-muted/10 px-4 py-6 sm:px-6 sm:py-8 xl:min-h-0 xl:overflow-hidden xl:border-t-0 xl:border-l xl:px-7 xl:py-8 2xl:px-8"
        >
          <TranscriptPlayer
