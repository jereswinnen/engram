...
vi.mock("./store", () => ({
  markReady: vi.fn(
    async (_ownerId: string, id: string, key: string, size: number) => {
      calls.ready = { id, key, size }
    }
  ),
  markError: vi.fn(async (_ownerId: string, id: string, err: string) => {
    calls.error = { id, err }
  }),
}))
vi.mock("@/lib/storage", () => ({
...
vi.mock("archiver", () => ({
  ZipArchive: class {
    entries: string[] = []
    _errorCbs: Function[] = []
    append(_body: unknown, opts: any) {
      this.entries.push(opts.name)
      calls.archive = this
    }
