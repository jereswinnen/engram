...
vi.mock("@/db", () => ({
  db: {
    ...
    update: () => ({
      set: (v: any) => ({
        where: async () => {
          Object.assign(rows[rows.length - 1], v)
        },
      }),
    }),
  },
...
describe("backup store", () => {
  it("creates a pending backup", async () => {
    ...
    const b = await createBackup("user-a")
  ...
  it("markReady sets status/key/size; markError sets error", async () => {
    ...
    const b = await createBackup("user-a")
    await markReady("user-a", b.id, "backups/x.zip", 123)
    const all = await getBackups("user-a")
    expect(all[0]).toMatchObject({
      status: "ready",
      storageKey: "backups/x.zip",
      sizeBytes: 123,
    })
  })
})
