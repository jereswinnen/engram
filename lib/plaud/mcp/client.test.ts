...
vi.mock("@/db", () => ({ db: {} }))
vi.mock("@/lib/config", () => ({
  config: { plaudRedirectUrl: () => "https://example.com/api/plaud/callback" },
}))
vi.mock("./auth-store", () => ({
  createPlaudAuthStore: vi.fn(),
  createPlaudOAuthAttempt: vi.fn(),
  consumePlaudOAuthAttempt: vi.fn(),
}))
...
describe("listFiles", () => {
  it("calls list_files and maps results", async () => {
    const client = {
      callTool: vi.fn(async () =>
        toolResult({ files: [{ id: "f1", start_at: "2026-06-01T10:00:00Z" }] })
      ),
    } as any
    const files = await listFiles(client, { date_from: "2026-05-01T00:00:00Z" })
    expect(client.callTool).toHaveBeenCalledWith({
      name: "list_files",
      arguments: { date_from: "2026-05-01T00:00:00Z", page: 1, page_size: 50 },
    })
  ...
  it("handles array / data / list response shapes", async () => {
    const client = {
      callTool: vi.fn(async () =>
        toolResult({ data: [{ id: "a", start_at: "2026-06-01T10:00:00Z" }] })
      ),
    } as any
    ...
    const client = {
      callTool: vi.fn(async () =>
        toolResult({
          id: "f1",
          start_at: "2026-06-01T10:00:00Z",
          presigned_url: "https://signed/f1.mp3",
        })
      ),
    } as any
