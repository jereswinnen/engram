...
describe("parseToolJson", () => {
  ...
  it("throws with raw MCP error text when result.isError is true", () => {
    const result = {
      isError: true,
      content: [{ type: "text", text: "MCP error: invalid arguments" }],
    }
...
describe("mapFile", () => {
  ...
  it("tolerates field-name variants and throws on no date", () => {
    expect(
      mapFile({ file_id: 2, start_time: "2026-06-02T00:00:00Z" }).fileId
    ).toBe("2")
  ...
  it("maps the presigned url when present", () => {
    expect(
      mapFileDetail({
        id: "f1",
        start_at: "2026-06-01T10:00:00Z",
        presigned_url: "https://signed/a",
      }).presignedUrl
    ).toBe("https://signed/a")
  })
  it("returns null (not a throw) when the audio isn't downloadable yet, so the sync can defer it", () => {
    expect(
      mapFileDetail({ id: "f1", start_at: "2026-06-01T10:00:00Z" }).presignedUrl
    ).toBeNull()
    expect(
      mapFileDetail({
        id: "f1",
