...
function rec(
  p: Partial<PlaudFile> & { fileId: string; startAtMs: number }
): PlaudFile {
...
vi.mock("./mcp/client", () => ({
  ...
  connect: vi.fn(async () => {
    if (calls.connectThrows) throw new Error("UnauthorizedError")
    return { close: vi.fn(async () => {}) }
  }),
  listFiles: vi.fn(async () => calls.files ?? []),
  getFile: vi.fn(async (_c: any, id: string) => ({
    fileId: id,
    name: id,
    startAtMs: 0,
    trashed: false,
    presignedUrl: `https://signed/${id}`,
  })),
...
vi.mock("@/lib/pipeline", () => ({
  runTranscription: vi.fn(async (_ownerId: string, id: string) => {
    calls.transcribed.push(id)
  }),
  runEnhancement: vi.fn(async (_ownerId: string, id: string) => {
    calls.enhanced.push(id)
  }),
}))
vi.mock("@/db", () => ({
  db: {
    insert: () => ({
      values: () => ({
        returning: async () => {
          const id = `rec-${calls.inserted.length}`
          calls.inserted.push(id)
          return [{ id }]
        },
      }),
    }),
    ...
    query: {
      recordings: {
        findMany: async () => calls.existing ?? [],
        findFirst: async () =>
          calls.findFirstResult ?? { status: "transcribed" },
      },
      syncState: {
        findFirst: async () =>
          calls.syncRow ?? {
...
beforeEach(() => {
  ...
  calls.findFirstResult = undefined
  calls.failUrl = undefined
  calls.deleted = []
  calls.failPut = undefined
  calls.connected = true
  calls.connectThrows = false
  calls.files = []
})
...
describe("syncPlaud", () => {
  ...
  const lastWriteWith = (key: string) =>
    calls.syncStateSet.filter((s: any) => key in s).at(-1)
  ...
  it("ingests new recordings through the pipeline and advances the checkpoint", async () => {
    ...
    const result = await syncPlaud("user-a")
    ...
    expect(new Date(lastWriteWith("lastSyncedAt").lastSyncedAt).getTime()).toBe(
      2000
    )
  ...
  it("skips when a sync is already running (recent runningSince), without processing", async () => {
    ...
    expect(lastWriteWith("lastResult").lastResult.note).toMatch(
      /already running/i
    )
  ...
  it("takes over a stale lock (older than the 10-min stale window)", async () => {
    calls.syncRow = {
      ...
      lastSyncedAt: null,
      lastResult: null,
      runningSince: new Date(Date.now() - 11 * 60 * 1000),
    }
    calls.files = [
      { fileId: "f1", name: "One", startAtMs: 1000, trashed: false },
    ]
  ...
  it("processes the whole batch concurrently and advances the checkpoint to the newest", async () => {
    calls.files = Array.from({ length: 12 }, (_, i) => ({
      fileId: `f${i}`,
      name: `R${i}`,
      startAtMs: (i + 1) * 1000,
      trashed: false,
  ...
  it("stops at the time budget and holds the checkpoint before the unprocessed (oldest) items", async () => {
    ...
    try {
      ...
      expect(
        new Date(lastWriteWith("lastSyncedAt").lastSyncedAt).getTime()
      ).toBe(999)
    } finally {
