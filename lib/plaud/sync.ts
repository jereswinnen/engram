...
async function getSyncRow(ownerId: string) {
  let row = await db.query.syncState.findFirst({
    where: ownerPredicate(syncState.ownerId, ownerId),
  })
  if (!row) {
    ;[row] = await db.insert(syncState).values({ ownerId }).returning()
...
export async function syncPlaud(ownerId: string): Promise<SyncResult> {
  ...
  const row = await getSyncRow(ownerId)
  ...
  if (
    row.runningSince &&
    Date.now() - new Date(row.runningSince).getTime() < STALE_LOCK_MS
  ) {
  ...
  try {
    if (!(await isConnected(ownerId))) {
      const result = {
        ...base,
        error: "not connected — connect Plaud in Settings",
      }
      await writeResult(row.id, result)
      return result
    }

    const checkpointMs = row.lastSyncedAt
      ? new Date(row.lastSyncedAt).getTime()
      : 0

    let client: Awaited<ReturnType<typeof connect>>
    try {
      client = await connect(ownerId)
    } catch (e) {
      console.error("[plaud sync] connect failed", e)
      const result = {
        ...base,
        error: "reconnect needed — Plaud authorization expired",
      }
    ...
    try {
      ...
      async function processOne(r: PlaudFile) {
        ...
        try {
          ...
          const key = buildAudioKey(
            rec.id,
            `x.${extFromContentType(contentType)}`
          )
          await getStorage().put(key, bytes, contentType)
          await db
            .update(recordings)
            .set({ storageKey: key })
            .where(ownedRecordingWhere(ownerId, rec.id))
          ...
          if (stored?.status === "error") {
            ...
            const msg =
              stored.errorMessage ?? "transcription/enhancement failed"
          ...
          if (insertedId) {
            try {
              await db
                .delete(recordings)
                .where(ownedRecordingWhere(ownerId, insertedId))
            } catch {}
          }
        }
      }
      ...
      const CONCURRENCY = Math.max(
        1,
        Number(process.env.PLAUD_SYNC_CONCURRENCY) || 5
      )
      const BUDGET_MS =
        Number(process.env.PLAUD_SYNC_BUDGET_MS) || 25 * 60 * 1000
      ...
      if (earliestUnprocessedMs !== Infinity) {
        ...
        console.warn(
          `[plaud sync] time budget (${Math.round(BUDGET_MS / 60000)}m) hit — ${left} item(s) deferred to next run`
        )
      }
      ...
      const earliestBlockerMs = Math.min(
        earliestFailureMs,
        earliestDeferredMs,
        earliestUnprocessedMs
      )
      const newCheckpointMs =
        earliestBlockerMs === Infinity
          ? maxSuccessMs
          : Math.max(checkpointMs, earliestBlockerMs - 1)
      const result: SyncResult = {
        ...
        ...(failedCount > 0 && firstItemError
          ? { error: `first failure: ${firstItemError}` }
          : {}),
      }
      await db
        .update(syncState)
        .set({ lastSyncedAt: new Date(newCheckpointMs), lastResult: result })
        .where(eq(syncState.id, row.id))
      console.info(
