...
describe("renderSnippet", () => {
  ...
  it("escapes around a mark too", () => {
    expect(renderSnippet(`<b>${mark("x")}</b>`)).toBe(
      "&lt;b&gt;<mark>x</mark>&lt;/b&gt;"
    )
  })
...
describe("renderPlainSnippet", () => {
  it("removes highlighting sentinels without treating text as HTML", () => {
    expect(renderPlainSnippet(`<b>${mark("x")}</b>`)).toBe("<b>x</b>")
  })
})
