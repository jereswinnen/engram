...
vi.mock("@/lib/recordings/store", () => ({
  getOwnedRecording: vi.fn(async () => ({ id: "rec1", ownerId: "user-a" })),
}))
    ...
    select: (_shape: any) => ({
      from: (_table: any) => ({
        innerJoin: (_joinTable: any, _on: any) => ({
          where: async (where: any) => {
            const f = parseWhere(where)
            const filtered = rsRows.filter(
              (r) => !f.recordingId || r.recordingId === f.recordingId
            )
            return filtered.map((rsRow) => {
              const speaker = speakersRows.find(
                (s) =>
                  s.id === rsRow.speakerId &&
...
describe("speakers store", () => {
  it("findOrCreateSpeaker: dedupes case-insensitively and preserves original casing", async () => {
    ...
    const r1 = await findOrCreateSpeaker("user-a", "Bjorn")
    const r2 = await findOrCreateSpeaker("user-a", " bjorn ")
  ...
  it("findOrCreateSpeaker: trims whitespace before storing", async () => {
    const { findOrCreateSpeaker } = await import("./store")
    const r1 = await findOrCreateSpeaker("user-a", "  Bob  ")
    const r2 = await findOrCreateSpeaker("user-a", "bob")
    expect(r1.id).toBe(r2.id)
  ...
  it("setRecordingSpeaker: blank name deletes the mapping", async () => {
    ...
    await setRecordingSpeaker("user-a", "rec1", "A", "Alice")
    expect(rsRows).toHaveLength(1)
    await setRecordingSpeaker("user-a", "rec1", "A", "  ")
    expect(rsRows).toHaveLength(0)
  })

  it("getRecordingSpeakerMap: returns { label: name } for a recording", async () => {
    const { setRecordingSpeaker, getRecordingSpeakerMap } =
      await import("./store")
    await setRecordingSpeaker("user-a", "rec1", "Speaker 1", "Alice")
    await setRecordingSpeaker("user-a", "rec1", "Speaker 2", "Bob")
    const map = await getRecordingSpeakerMap("user-a", "rec1")
  ...
  it("getRecordingSpeakerMap: returns empty object when no mappings exist", async () => {
    ...
    const map = await getRecordingSpeakerMap("user-a", "rec-none")
    expect(map).toEqual({})
  })

  it("listSpeakers: returns all speakers", async () => {
    const { findOrCreateSpeaker, listSpeakers } = await import("./store")
    await findOrCreateSpeaker("user-a", "Alice")
    await findOrCreateSpeaker("user-a", "Bob")
    const list = await listSpeakers("user-a")
